...
void nand_read_in_page(unsigned char *buffer,unsigned long srcAddr,int size)
{
    int index;

    //如果读取内容不再一个页中，直接返回
    if((srcAddr & (~NAND_PAGE_MASK)) != ((srcAddr + size - 1) & (~NAND_PAGE_MASK)))
      return; 
    //选中芯片
    NAND_ENABLE_CE();

    //发送地址
    nand_write_address(srcAddr);
...
void copy2addr(unsigned long srcAddr,unsigned char* aim,int size)
{
    ...
    if(srcAddr & NAND_PAGE_MASK)
    {
        ...
        nand_read_in_page(aim,srcAddr,length);
        srcAddr += length;
        size -= length; 
    }
    ...
    {
       length = (size / NAND_PAGE_SIZE)*NAND_PAGE_SIZE;
       nand_read_page(aim,srcAddr,length);
       nand_read_in_page(aim,srcAddr,size - length);
    }
    else
